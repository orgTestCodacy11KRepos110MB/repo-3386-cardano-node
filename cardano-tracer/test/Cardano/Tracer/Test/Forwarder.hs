...
doConnectToAcceptor
  ...
  -> IO ()
doConnectToAcceptor TestSetup{..} snocket makeBearer address timeLimits (ekgConfig, tfConfig, dpfConfig) = do
  store <- EKG.newStore
...
doListenToAcceptor TestSetup{..}
  snocket makeBearer address timeLimits (ekgConfig, tfConfig, dpfConfig) = do
